...
static string GetSeasonByMonth(int month)
{
    ...
    string season = null;
    if(month == DecMonth || month == JanMonth || month == FebMonth)
    {
        season = Winter;
    }
    else if (month == MarchMonth || month == AprilMonth || month == MayMonth)
    {
        season = Spring;
    }
    else if (month == JuneMonth || month == JulyMonth || month == AugustMonth)
    {
        season = Summer;
    }
    else if (month == SeptMonth || month == OctMonth || month == NovMonth)
    {
        season = Autumn;
    }
